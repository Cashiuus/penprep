#!/usr/bin/env bash
## =============================================================================
# File:     bootstrap-ctf.sh
# Author:   Cashiuus
# Created:  03-Feb-2022     Revised:
#
##-[ Info ]---------------------------------------------------------------------
# Purpose:  Some basic setup of tools needed for a certain CTF.
#
#
#
##-[ Links/Credit ]-------------------------------------------------------------
...
$SUDO apt-get -y install curl gcc git wget tmux




# Install radare - https://github.com/radareorg/radare2
mkdir -p ~/tools
cd ~/tools
...
# Requires JDK 11 x64 - but luckily this is what comes w/ the latest Kali 2021.4a VMware VM
cd ~/tools
curl -SL 'https://github.com/NationalSecurityAgency/ghidra/releases/download/Ghidra_10.1.2_build/ghidra_10.1.2_PUBLIC_20220125.zip' -o ghidra.zip
unzip ghidra.zip
if [[ $? -eq 0 ]]; then
  rm -rf ghidra.zip
fi
...
# Run it via:
#./ghidraRun
echo -e "${GREEN}[*]${RESET} Ghidra reversing framework installed. Run it from terminal via: ${BLUE}~/tools/ghidra/ghidraRun & ${RESET}"





# CyberChef & Awesome-CTF
echo -e "${GREEN}[*]${RESET} Opening CyberChef web application toolkit and Awesome-CTF project. be sure to bookmark them for the CTF!"Bsleep 3s
firefox --new-tab "https://gchq.github.io/CyberChef/" &
sleep 2s
firefox --new-tab "https://github.com/apsdehal/awesome-ctf" &
sleep 1s
firefox --new-tab "https://github.com/uppusaikiran/awesome-ctf-cheatsheet" &
sleep 1s
firefox --new-tab "https://www.villageb.io/capturetheflag" &


echo -e "${GREEN}[*]${RESET} Setup complete. Look in your home directory under ${BLUE}${HOME}/tools${RESET} for non-standard tools that were installed."
